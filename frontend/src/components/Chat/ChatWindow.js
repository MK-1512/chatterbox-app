...
const ChatWindow = ({ chatRoom }) => {
    ...
    const { messages, typingUsers, isConnecting, sendMessage, sendTypingNotification, sendReadReceipt } = useWebSocket(chatRoom?.id);
            ...
            <div className="message-list">
                {messages.map((msg) => (
                    <Message 
                        key={msg.id} 
                        message={msg} 
                        isMine={msg.author.username === user.username}
